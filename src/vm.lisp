...
(defun make-vm (&optional (nom 'maVM) (taille 10000))
	...
	; (print "VM initialisée avec succès !")
)

(defun make-vm-other (name size)
    ;; registres
    (setf (get name 'PC) 0)
    (setf (get name 'BP) 0)
    (setf (get name 'FP) 0)
    (setf (get name 'SP) -1)
    (setf (get name 'R0) nil)
    (setf (get name 'R1) nil)
    (setf (get name 'R2) nil)

    ;; drapeaux
    (setf (get name 'DE) nil)
    (setf (get name 'DPP) nil)
    (setf (get name 'DPG) nil)

    ;; mémoire, utilisée comme une pile
    (setf (get name 'mem) (make-array size))

    ;; table de références
    (setf (get name 'refs) (make-hash-table)))
